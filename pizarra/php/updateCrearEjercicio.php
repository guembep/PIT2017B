<?php
	...
	if(isset($_SESSION['id'])){
		...
		$stmt = $db->prepare("UPDATE `ejercicios` SET (`deporte`, `idequipo`, `categoria`, `subcategoria`, `nombre`, `explicacion`, `duracion`, `material`, `personasmin`, `personasmax`, `foto`, `idusuario` , `datospizarra`) WHERE `id` VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		
		//echo 'Deporte: ' .$sport;
	    $stmt->bind_param('sissssssiisisi', $sport, $club, $category, $subcategory, $name, $description, $duration, $material, $personmin, $personmax, $imagename, $iduser, $datospizarra, $idejercicio);
